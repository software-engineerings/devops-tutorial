var DevopsSchool_googlemap_init_obj = {};
var DevopsSchool_GEOCODE_ERROR = "Error";
// Google map Styles
var DevopsSchool_googlemap_styles = {
    'default': [],
...
function googlemap_init(dom_obj, coords) {
    ...
    try {
        ...
        var id = dom_obj.id;
        DevopsSchool_googlemap_init_obj[id] = {};
        DevopsSchool_googlemap_init_obj[id].dom = dom_obj;
        DevopsSchool_googlemap_init_obj[id].point = coords.point;
        DevopsSchool_googlemap_init_obj[id].description = coords.description;
        DevopsSchool_googlemap_init_obj[id].title = coords.title;
        DevopsSchool_googlemap_init_obj[id].opt = {
            zoom: coords.zoom,
            center: latlng,
            scrollwheel: false,
            scaleControl: true,
            ...
            styles: DevopsSchool_googlemap_styles[coords.style ? coords.style : 'default'],
        ...
        custom_map.geocode(coords.latlng!=='' ? {'latLng': latlng} : {"address": coords.address}, function(results, status) {
            if (status == google.maps.GeocoderStatus.OK) {
                DevopsSchool_googlemap_init_obj[id].address = results[0].geometry.location;
                googlemap_create(id);
            } else {
                alert(DevopsSchool_GEOCODE_ERROR + ' ' + status);
            }
        });

        jQuery(window).resize(function() {
            if (DevopsSchool_googlemap_init_obj[id].map) {
                DevopsSchool_googlemap_init_obj[id].map.setCenter(DevopsSchool_googlemap_init_obj[id].address);
            }
        });
...
function googlemap_create(id) {
    "use strict";
    if (!DevopsSchool_googlemap_init_obj[id].address) { 
        return false;
    }
    DevopsSchool_googlemap_init_obj[id].map = new google.maps.Map(DevopsSchool_googlemap_init_obj[id].dom, DevopsSchool_googlemap_init_obj[id].opt);
    DevopsSchool_googlemap_init_obj[id].map.setCenter(DevopsSchool_googlemap_init_obj[id].address);
    var markerInit = {
        map: DevopsSchool_googlemap_init_obj[id].map,
        position: DevopsSchool_googlemap_init_obj[id].address   //DevopsSchool_googlemap_init_obj[id].map.getCenter()
    };
    if (DevopsSchool_googlemap_init_obj[id].point) { 
        markerInit.icon = DevopsSchool_googlemap_init_obj[id].point;
    }
    if (DevopsSchool_googlemap_init_obj[id].title) { 
        markerInit.title = DevopsSchool_googlemap_init_obj[id].title;
    }
    var marker = new google.maps.Marker(markerInit);
    var infowindow = new google.maps.InfoWindow({
        content: DevopsSchool_googlemap_init_obj[id].description
    });
    google.maps.event.addListener(marker, "click", function() {
        infowindow.open(DevopsSchool_googlemap_init_obj[id].map, marker);
    });
}


function googlemap_refresh(){
    for(id in DevopsSchool_googlemap_init_obj){
        googlemap_create(id);
    }
}
